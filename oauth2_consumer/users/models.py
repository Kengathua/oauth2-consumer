...
class UserManager(BaseUserManager):
    ...
    def create_superuser(self, email, first_name, last_name, password=None, **fields):
        ...
        user = self.create_user(email, password=password, first_name=first_name, last_name=last_name)    # noqa
        user.is_admin = True
        user.is_staff = True
...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    email = models.EmailField(
        verbose_name='email address', null=False, blank=False, max_length=255, editable=True, unique=True)   # noqa
